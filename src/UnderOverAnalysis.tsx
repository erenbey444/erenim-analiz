...
export default function UnderOverAnalysis(){
  ...
  }).filter(x=>x.total>=minSample),[matches,oddsHistory,mode,tolerance,minSample]);
  ...
  return <div className="uo-page">
    ...
    {loading?<div className="uo-loading"><LoaderCircle className="spin"/> Geçmiş veriler hazırlanıyor…</div>:<div className="uo-list">{sorted.map(a=>{
      ...
      {a.total?<><div className="uo-total"><strong>{a.total} Geçmiş Maç Bulundu</strong><em>{mode==='exact'?'Birebir':'Yakın eşleşme'}</em></div><div className="uo-stats"><div><span>ALT</span><b>{a.under} (%{ur})</b><i><u style={{width:`${ur}%`}}/></i></div><div><span>ÜST</span><b>{a.over} (%{or})</b><i><u style={{width:`${or}%`}}/></i></div></div><button className="uo-show" onClick={()=>setOpened(isOpen?null:a.id)}>{isOpen?<ChevronUp/>:<ChevronDown/>} {isOpen?'Geçmiş Maçları Gizle':'Geçmiş Maçları Göster'}</button>{isOpen&&<div className="uo-table"><table><thead><tr><th>Tarih</th><th>Lig</th><th>Maç</th><th>Alt</th><th>Üst</th><th>Skor</th><th>Gol</th><th>Sonuç</th></tr></thead><tbody>{a.rows.map((r,i)=>{const g=goals(r.score);return <tr key={`${r.date}-${r.home}-${i}`}><td>{r.date}</td><td>{r.league}</td><td>{r.home} – {r.away}</td><td>{r.under25?.toFixed(2)}</td><td>{r.over25?.toFixed(2)}</td><td>{r.score}</td><td>{g}</td><td><b className={g>2.5?'uo-over':'uo-under'}>{g>2.5?'ÜST':'ALT'}</b></td></tr>})}</tbody></table></div>}</>:<div className="uo-empty">Geçmiş eşleşme bulunamadı</div>}</article>})}{!sorted.length&&<div className="uo-empty-page">Seçili filtrelere uygun bugünkü maç bulunamadı.</div>}</div>}
  </div>
}
